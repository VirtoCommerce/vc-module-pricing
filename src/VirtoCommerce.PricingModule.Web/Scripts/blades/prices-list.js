    ...
    .factory('virtoCommerce.pricingModule.priceValidatorsService', [function () {
        ...
        return {
            ...
            isUniqueQty: function (price) {
                ...
                return Math.round(price.minQuantity) > 0 &&
                    _.all(allPrices, function (x) {
                        return x === price ||
                            Math.round(x.minQuantity) !== Math.round(price.minQuantity) ||
            ...
            isUniqueQtyForPricelist: function (price) {
                // Disable unique quantity test when time filtering is used.
                return Math.round(price.minQuantity) > 0 &&
                    _.filter(allPrices, function (x) {
                        return x.pricelistId === price.pricelistId &&
                            x.minQuantity === price.minQuantity &&
                            !x.startDate &&
                            !x.EndDate &&
                            !price.startDate &&
                            !price.endDate;
